...
/**************** Banner Text widgets ***************************/
class apporypro_banner_with_text_widget extends WP_Widget {
	function __construct() {
		$widget_ops  = array('classname' => 'apporypro-banner-with-text-widget', 'description' => __('Display  Banner Text widgets for AppOryPro Template', 'apporypro'));
		$control_ops = array('width' => 200, 'height' => 250);
		parent::__construct(false, $name = __('TF: Banner with Text', 'apporypro'), $widget_ops, $control_ops);
	}
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'banner_type' ); ?>"><?php _e( 'Fullwidth Banner (This feature works only with Single Column Banner is On):', 'apporypro' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'banner_type' ); ?>" name="<?php echo $this->get_field_name( 'banner_type' ); ?>">
				<option value="on" <?php selected( $instance['banner_type'], 'on'); ?>><?php _e( 'On', 'apporypro' ); ?></option>
				<option value="off" <?php selected( $instance['banner_type'], 'off'); ?>><?php _e( 'Off', 'apporypro' ); ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'single_column' ); ?>"><?php _e( 'Single Column Banner:', 'apporypro' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'single_column' ); ?>" name="<?php echo $this->get_field_name( 'single_column' ); ?>">
				<option value="on" <?php selected( $instance['single_column'], 'on'); ?>><?php _e( 'On', 'apporypro' ); ?></option>
				<option value="off" <?php selected( $instance['single_column'], 'off'); ?>><?php _e( 'Off', 'apporypro' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'light_text_color' ); ?>"><?php _e( 'Text Color Light:', 'apporypro' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'light_text_color' ); ?>" name="<?php echo $this->get_field_name( 'light_text_color' ); ?>">
				<option value="on" <?php selected( $instance['light_text_color'], 'on'); ?>><?php _e( 'On', 'apporypro' ); ?></option>
				<option value="off" <?php selected( $instance['light_text_color'], 'off'); ?>><?php _e( 'Off', 'apporypro' ); ?></option>
			</select>
		</p>
		<hr>
		...
		?>
			<p>
				<label for="<?php echo $this->get_field_id(key($defaults));?>">
					<?php _e('Page # ', 'apporypro'); echo absint($i);?>
				:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id($button_text);?>">
					<?php _e('Button Text #', 'apporypro'); echo absint($i);?>
				</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id($button_link);?>">
					<?php _e('Button Url #', 'apporypro'); echo absint($i);?>
				</label>
	...
	function widget($args, $instance) {
		...
		<section class="widget apporypro-banner-with-text-widget <?php if ($banner_type =='on'){ echo ' full-width-banner '; } if ($single_column =='on'){ echo ' single-column-banner '; } ?>">
		...
		</section> <!-- end .apporypro-banner-with-text-widget -->
	<?php }
}